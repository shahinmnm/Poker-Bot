...
async def test_delete_lobby_removes_primary_key(poker_model, kv_store):
    ...
    # Verify key deleted
    assert not kv_store.exists("private_game:" + game_code)
...
async def test_delete_lobby_removes_user_mappings(poker_model, kv_store):
    ...
    for pid in player_ids:
        kv_store.set("user:" + str(pid) + ":private_game", game_code)
    ...
    for pid in player_ids:
        assert not kv_store.exists("user:" + str(pid) + ":private_game")
...
async def test_fallback_to_context_when_json_missing(poker_model, kv_store):
    ...
    kv_store.set("user:999:private_game", game_code)
...
async def test_partial_cleanup_on_redis_error(poker_model, kv_store):
    ...
    kv_store.set("user:100:private_game", game_code)
    kv_store.set("user:200:private_game", game_code)
    ...
    assert not kv_store.exists("user:200:private_game")
...
async def test_handles_bytes_game_code(poker_model, kv_store):
    ...
    kv_store.set("user:100:private_game", game_code_str)

    # Call with bytes
    await poker_model.delete_private_game_lobby(chat_id, game_code_bytes)

    # Should still clean up
    assert not kv_store.exists("private_game:" + game_code_str)
    assert not kv_store.exists("user:100:private_game")

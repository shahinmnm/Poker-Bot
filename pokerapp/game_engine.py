...
from pokerapp.entities import (
    Game, GameState, Player, PlayerState, PlayerAction
)
...
class PokerEngine:
    ...
    def validate_join_balance(self, player_balance: int, table_stake: int) -> bool:
        """
    ...
    def should_end_round(self, game: Game) -> bool:
        ...
        at_round_initiator = (current_player.user_id == game.trading_end_user_id)
        
        return all_matched and at_round_initiator
    
    def process_turn(self, game: Game) -> TurnResult:
        """
        Process one player turn iteration (REPLACES RECURSIVE _process_playing).
        
        Returns:
            TurnResult indicating whether to continue, end round, or end game
        """
    ...
    def advance_to_next_street(self, game: Game) -> GameState:
        """
        Transition to next betting round (flop → turn → river → showdown).

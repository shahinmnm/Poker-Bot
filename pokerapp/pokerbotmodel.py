...
class PokerBotModel:
    def __init__(
        ...
        self._stake_config = STAKE_PRESETS.get(
            self._cfg.DEFAULT_STAKE_LEVEL,
            STAKE_PRESETS["micro"],
        )
    ...
    async def _start_game(
        ...
        self._coordinator.prepare_pre_flop(game, self._stake_config)
    ...
    async def _start_betting_round(self, game: Game, chat_id: int) -> None:
        ...
        while True:
            ...
            if result == TurnResult.END_ROUND:
                ...
                _, cards_count = self._coordinator.advance_game_street(game)
                if cards_count > 0:
                    await self.add_cards_to_table(cards_count, game, chat_id)
                continue

            if result == TurnResult.CONTINUE_ROUND and next_player:
                if next_player.wallet.value() <= 0:
                    next_player.state = PlayerState.ALL_IN
                    continue

                if next_player.state != PlayerState.ACTIVE:
                    continue
    ...
    async def call_check(
        ...
        try:
            ...
            self._coordinator.call_or_check(game, player)
        except UserException as e:
            await self._view.send_message(chat_id=chat_id, text=str(e))
            return

        await self._start_betting_round(game=game, chat_id=chat_id)
    ...
    async def raise_rate_bet(
        ...
        try:
            ...
            self._coordinator.raise_bet(game, player, raise_bet_rate.value)
        except UserException as e:
            await self._view.send_message(chat_id=chat_id, text=str(e))
            return
    ...
    async def all_in(
        ...
        amount = self._coordinator.all_in(game, player)
        await self._view.send_message(
            chat_id=chat_id,
